...
```{r}
test_long_wo_ff <- test_long %>% 
  filter(ship_genders != "female_female")

poisson_long_test <- glmer(violence_count ~ violence_cat*ship_genders + fandom_label + (1|author), data = test_long_wo_ff, family = poisson, nAGQ = 100)
